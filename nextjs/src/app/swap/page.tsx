...
const PAIR_ASSET_WBNB = getAddress("0x5a1175DdB5094B0f4FdD4475e53d22acfF10e8f7");
...
/* -------------------- Helpers -------------------- */
const isWrap   = (from: TokenInfo, to: TokenInfo) => from.native && !to.native && getAddress(to.address!) === WBNB;
const isUnwrap = (from: TokenInfo, to: TokenInfo) => !from.native && getAddress(from.address!) === WBNB && to.native;

/* -------------------- Page -------------------- */
export default function SwapPage() {
  const { address } = useAccount();
  const { call, txHash, isPending, isMining, isConfirmed } = useContractWriter();
  ...
  /* ---- Balances ---- */
  const nativeBal = useBalance({ address, query: { enabled: Boolean(address) } });
  ...
  }, [address, from.native, nativeBal.data?.value, fromErc20Raw, from.decimals]);

  /* ---- Pair data ---- */
  const { data: t0 } = useReadContract({ abi: pairAbi, address: PAIR_ASSET_WBNB, functionName: "token0" });
  const { data: t1 } = useReadContract({ abi: pairAbi, address: PAIR_ASSET_WBNB, functionName: "token1" });
  const { data: reservesRaw } = useReadContract({ abi: pairAbi, address: PAIR_ASSET_WBNB, functionName: "getReserves" });
  ...
  const pair = useMemo(() => {
    try {
      ...
      const isWbnbAsset = (token0 && token1) &&
        ((token0 === WBNB && token1 === ASSET) || (token0 === ASSET && token1 === WBNB));
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!pair || !pair.isWbnbAsset) { setRouteMsg("Pair is not ASSET/WBNB or reserves not loaded."); return; }
        ...
        if (!valid) { setRouteMsg("Unsupported route (this file only handles ASSET/WBNB pair)."); return; }
        ...
        if (out === 0n) { setRouteMsg("Insufficient liquidity or wrong fee assumption."); return; }
  ...
  const swap = async () => {
    try {
      ...
      if (!amountUi || Number(amountUi) <= 0) throw new Error("Enter a valid amount.");
      if (!pair || !pair.isWbnbAsset) throw new Error("Pair not ready (ASSET/WBNB).");

      if (Number(amountUi) > fromBalance) throw new Error("Amount exceeds your balance.");
      ...
      const willUnwrapAfter =
        (fromIsAsset && to.native) ? true : false;
      ...
      if ((effectiveFrom.address && getAddress(effectiveFrom.address) === WBNB) && (toIsAsset || willUnwrapAfter === false)) {
        ...
        // output = ASSET
        if (isToken0Wbnb) { amount0Out = 0n; amount1Out = out; }
        else              { amount0Out = out; amount1Out = 0n; }
      }
      // Case: ASSET -> WBNB
      else {
        ...
        if (out < min) throw new Error("Slippage too high.");
        // output = WBNB
        if (isToken0Wbnb) { amount0Out = out; amount1Out = 0n; }
        else              { amount0Out = 0n; amount1Out = out; }
      }
      ...
      setUiError(msg.replace(/0x[a-fA-F0-9]{20,}/g, (m: string) => m.slice(0, 10) + "…"));
    }
  };
          ...
          <div className="cardContent">
            ...
            <div className="fieldGroup">
              <div className="labelRow">
                <span>From</span>
                <span>Balance: {fromBalance.toFixed(6)} {from.symbol}</span>
              </div>
              <div className="inputWrap">
                <select
                  ...
                  value={from.symbol}
                  onChange={(e) => {
                    const t = TOKENS.find(x => x.symbol === e.target.value as any)!;
                    setFrom(t); clearError();
                  }}
                >
                  {TOKENS.map(t => (
                ...
                <input
                  ...
                  placeholder="0.0"
                  value={amountUi}
                  onChange={(e) => { setAmountUi(e.target.value); clearError(); }}
                  className="input"
                />
              </div>
            </div>
            ...
            <div className="switchRow">
              <button
                onClick={() => {
                  const tmp = from; setFrom(to); setTo(tmp); clearError();
                }}
                className="switchBtn"
                title="Switch tokens"
              />
            ...
            <div className="fieldGroup">
              ...
              {/* Amount preview */}
              {routeMsg && (
                <small style={{ display: "block", marginTop: 6, color: "tomato" }}>
                  {routeMsg}
                </small>
              )}
              {(!routeMsg && rawOut !== null) && (
                <small style={{ display: "block", marginTop: 6, opacity: 0.8 }}>
                  You would get: <b>{estOut} {to.symbol}</b> · Min received (slippage {slippageBps/100}%): <b>{minOut} {to.symbol}</b>
                </small>
              )}
            </div>
            ...
            <div style={{ marginTop: 10 }}>
              ...
              {uiError && (
                <small style={{ color: "tomato", display: "block", marginTop: 8 }}>
                  {uiError}
                </small>
              )}
            </div>
